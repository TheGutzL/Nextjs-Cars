...
import {
  ...
  FormAddCarSchema,
  formAddCarSchema,
} from "./FormAddCar.form";
import { FormAddCarProps } from "./FormAddCar.types";

const FormAddCar = ({ setOpenDialog }: FormAddCarProps) => {
  const { toast } = useToast();
  const [photoUploaded, setPhotoUploaded] = useState(false);
  const router = useRouter();

  const form = useForm<FormAddCarSchema>({
    resolver: zodResolver(formAddCarSchema),
    defaultValues,
  });

  const onSubmit: SubmitHandler<FormAddCarSchema> = async (values) => {
    setOpenDialog(false);
    try {
